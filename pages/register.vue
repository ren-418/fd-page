<template>
  <div class="md:container mx-0 lg:mx-auto pt-5">
    <div class="grid grid-cols-1 md:grid-cols-2 p-0 gap-10 items-center lg:pl-10 lg:pr-32 w-full">
      ...
      <div class="w-full h-auto bg-none md:bg-white p-5 rounded-xl flex flex-col gap-5 md:shadow-xl card-height">
        <h1 class="text-2xl">
        ...
        <div class="flex flex-col gap-2 md:gap-1">
          <div class="flex flex-col md:flex-row gap-2 w-full justify-between">
            <div class="icon-prefix-input text-base w-full">
              <Icon name="user" />
              <input v-model="form.first_name" class="form-control font-normal border w-full p-2" type="text"
                name="name" autocomplete="username" placeholder="First name" maxlength="128" required
                :onchange="(e: any) => onInput(e, 'first_name', 'isVaildName')" />
            </div>
            <div class="icon-prefix-input text-base w-full">
              <Icon name="user" />
              <input v-model="form.last_name" class="form-control font-normal border w-full p-2" type="text" name="name"
                autocomplete="username" placeholder="Last name(optional)" maxlength="128" />
            </div>
          </div>
          ...
          <div v-if="isEmailUse" class="icon-prefix-input text-base">
            <Icon name="email" />
            <input v-model="form.email" type="email" name="email" class="form-control font-normal border w-full p-2"
          ...
          <div v-if="isEmailUse" class="flex justify-end items-center gap-2 md:gap-3">
          ...
          <div class="flex flex-col gap-2 md:gap-1">
            <div class="icon-prefix-input text-base">
              <Icon name="user" />
              <input v-model="form.username" class="form-control font-normal border w-full p-2 indent-3" type="text"
                name="username" autocomplete="username" placeholder="Choose username" maxlength="128" required
                :onchange="(e: any) => onInput(e, 'username', 'isVaildUsername')" />
            </div>
            <div style="color: red" v-if="!validateStatus?.isVaildUsername?.status">
              {{ validateStatus?.isVaildUsername?.msg }}
            </div>
            <div class="icon-prefix-input text-base">
              <Icon name="lock" />
              <input v-model="form.password" class="form-control font-normal border w-full p-2 indent-3" type="password"
                name="password" autocomplete="password" placeholder="Choose password" maxlength="128" required
                :onchange="(e: any) => onInput(e, 'password', 'isStrongPassword')" />
            </div>
            <div style="color: red" v-if="!validateStatus?.isStrongPassword?.status">
          ...
          <div class="text-color-3">
            ...
            <div class="w-95 inline">
              ...
              <NuxtLink target="_blank" class="text-color-2">Terms of use</NuxtLink>
              and
              <NuxtLink class="text-color-2 indent-3" target="_blank">Privacy policy</NuxtLink>
            </div>
          </div>
...
const handleSignUp = async () => {
  ...
  const formData = {
    custom_gender: "",
    date_of_birth: "2012-01-05T16:00:00.000Z",
